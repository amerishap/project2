...
function Stars(pos) {
  ...
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 7;
}
Stars.prototype.type = "stars";
...
var wobbleSpeed = 8, wobbleDist = 0.08;
...
Stars.prototype.act = function(step) {
  this.wobble += step * starsspeed;
  var starsPos = Math.sin(this.wobble) * starsDist;
  var starsPosup = Math.cos(this.wobble) * starsDist;
  this.pos = this.basePos.plus(new Vector(0, starsPos, starsPosup));
};
...
} else if (type == "coin" || type == "boost" || type == "stars") {
    this.actors = this.actors.filter(function(other) {
      return other != actor;
    });
    // If there aren't any coins left, player wins
    if (!this.actors.some(function(actor) {
           return actor.type == "stars";
         })) {
      this.status = "won";
      this.finishDelay = 1;
	  console.log("You won!");
